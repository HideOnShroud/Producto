...
const HomePage = () => {
    ...
    const [iteme, setItems] = useState(items)
    useEffect(() => {
        getItems()
    }, [getItems])
    ...
    return (
        <>
            ...
            <div className="flex flex-auto pt-20 flex-row bg-neutral-50 justify-center flex-wrap gap-10 pl-20 pr-20">
                {iteme.map((item) => <div key={item.sku} className={select.includes(item.sku) ? "opacity-50 relative" : "relative"} onClick={() => handleClick(item.sku)}>
